...
export default function CollectionIndex() {
  ...
  /* --------------------------------------------
  * USEEFFECT TO RESET PAGE ON APPLYING FILTER
    -------------------------------------------- */
  useEffect(() => {
    setPages(1);
            ...
            {({ index, style }) => (
              <div style={style} className="p-2">
                <PaginatedGallery
                  ...
                  disablePagination
                  page={pages}
                  onPageChangeCallback={setPages}
                />
              </div>
            )}
          </List>
        ) : (
          <PaginatedGallery data={filteredProducts} 
          itemsPerPage={8} page={pages} onPageChangeCallback={setPages}/>
        )}
      </div>
    </div>
  );
